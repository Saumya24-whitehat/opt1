...
function PreviousDayHighAndLow(PineJS) {
  return {
    name: "PreviousDayHighAndLow",
    metainfo: {
      _metainfoVersion: 0,
      id: "PreviousDayHighAndLow@tv-basicstudies-1",
      scriptIdPart: "",
      description: "Previous Day High And Low (iCharts)",
      shortDescription: "Previous Day High and Low (iCharts)",
      is_hidden_study: !1,
      is_price_study: 1,
      isCustomIndicator: !1,
      trackPrice: true,
      ...
      styles: {
        plot_0: { title: "High", histogramBase: 0, joinPoints: !1 },
        plot_1: { title: "Low", histogramBase: 0, joinPoints: !1 },
      },
      inputs: [],
    },
    constructor: function () {
      this.init = async function (context, inputCallback) {
        this._context = context;
        this._input = inputCallback;
        let symbol = PineJS.Std.ticker(this._context) + "_PDHL";
      ...
      this.main = function (ctx, inputs) {
        ...
        // // console.log("High: ");
        // const alignedHigh = high.adopt(secondarySymbolTime, mainSymbolTime, 1);
        // const alignedLow = low.adopt(secondarySymbolTime, mainSymbolTime, 1);
        // this._context.select_sym(0);
        const high = PineJS.Std.high(this._context);
        const low = PineJS.Std.low(this._context);

        return [parseFloat(high), parseFloat(low)]


        if ((!isNaN(alignedHigh) || !isNaN(alignedLow)) && (alignedHigh > 0 && alignedLow > 0)) {
          return [parseFloat(alignedHigh), parseFloat(alignedLow)];
        }
      }
    }
  }
